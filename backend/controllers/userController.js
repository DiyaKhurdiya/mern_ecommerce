...
exports.googleLogin = async(req, res) => {
  try {
    ...
    console.log(verify);
    if(!email_verified) return res.status(401).json({msg: "Email verification failed."});
    ...
    const user = await User.findOne({email});
      ...
      if(user) {
        ...
        const newUser = await User.create({
          ...
          avatar: {
            public_id:  "sample id",
          url: "https://lh3.googleusercontent.com/a-/AOh14GgOlOc5fC7Dk6ykBY47xhOKrW-lit4nMTMUwl5RMg=s96-c",
        }
      ,
    });
      //   new User({
